...
export const generateGraphFromUserInput = (
  cy: cytoscape.Core,
  input: string
) => {
  ...
  lines.forEach((line, index) => {
    ...
    const verifyWeight = (weight: any) => {
      const parsedWeight = parseInt(weight);
      if (Number.isNaN(parseInt(weight))) {
        return 0;
      } else {
        return Math.abs(parsedWeight);
      }
    };
